...
function openModal(recipeId) {
    ...
    if (recipeId) {
        try {
            fetch(`/api/personalise-reicipe/${recipeId}`)
                ...
                .then(data => {
                    ...
                    updateMacroFields(data['macros']['nutrients'].filter(nutrient => {
        ...
        // fetch(`https://api.spoonacular.com/recipes/${id}/information?apiKey=2b74ec79341f45c2a24fa427b5e79fcf&includeNutrition=true`)
        //     .then(response => response.json())
        //     .then(data => {
        //         let { cheap, dairyFree, vegetarian, vegan, instructions, summary } = data["recipes"][0];
        //         data = { cheap, dairyFree, vegetarian, vegan, instructions, summary }
        //         updateExternalApiFields(data);
        //         console.log(data);
        //     });
        let {cheap, dairyFree, vegetarian, vegan, instructions, summary, nutrition} = recipeExample;
        recipeExample = {cheap, dairyFree, vegetarian, vegan, instructions, summary, nutrition}
        updateExternalApiFields(recipeExample);
        let nutrients = nutrition['nutrients'].filter(nutrient => {
            return nutrient.name === "Protein" || nutrient.name === "Fat" || nutrient.name === "Calories";
        });
        updateMacroFields(nutrients);
        // console.log(recipeExample);
        console.log(nutrients);
        // let recipeExample = recipe["recipes"][0];
        // let nutrientsExample = recipe["recipes"][0];
        // updateExternalApiFields(recipeExample);
    ...
    "analyzedInstructions": [
        ...
        },
        {
            ...
            "steps": [
                {
                    "number": 1,
                    "step": "Pudding can be chilled up to 1 day.",
                    "ingredients": [],
                    "equipment": []
                }
            ]
